...
import { t, getLocale, getDateLocale, getLanguageLabel, getPublicTrackingError, resolveLocaleFromUrl, readStoredLocale, persistLocale, setLocale, initLocale, registerLocaleChangeHandler, applyStaticTranslations, DEFAULT_LOCALE, STORAGE_KEY } from './i18n.js';
...
async function refreshAccountContactsFromSupabase({ silent = true } = {}) {
  ...
  try {
    ...
    if (!silent) showAuthMessage(`Σφάλμα Supabase: ${message}`, true);
    renderContacts();
    throw error;
  } finally {
...
sosContactList?.addEventListener('click', async (event) => {
  ...
  if (action) {
    ...
    logSosNotification(contact, action.dataset.sosMethod, 'Άνοιξε');
  }
});
